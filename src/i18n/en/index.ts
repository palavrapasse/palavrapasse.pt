import type { Translation } from '../i18n-types';

const en: Translation = {
	homepageTitle: 'Are my credentials leaked?',
	homepageDescription:
		'Use our search engine and find out if your credentials are floating in the web',
	notifyButtonTooltip: 'Notify me',
	notifyButtonModalTitle: 'Notify me',
	notifyButtonModalDescription:
		'Do you want to receive e-mail notifications if your credentials get leaked?',
	leakContext: 'Context',
	leakShareDate: 'Leak Date',
	leaksQueryNotFoundResponse: "Can' find leaked records for: {email}",
	yes: 'Yes',
	no: 'No'
};

export default en;
